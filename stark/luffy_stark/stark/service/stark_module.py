...
def get_choices_field(title, field):
	...
	def inner(self, is_header=None, obj=None):
		if is_header:
			return title
...
class BaseHandler:
	...
	def display_mul_op(self, obj=None, is_header=None):
		"""
		批量操作列
		"""
		if is_header:
	...
	def get_add_btn(self):
		...
		"""
		if self.has_add_btn:
			return "<a class='btn btn-success btn-sm' href='%s'>添加</a>" % self.get_reverse_add_url()
		return None
	...
	def show_view(self, request, *args, **kwargs):
		...
		print(search_dict)
		query_set = self.model_class.objects.filter(conn).filter(**search_dict).order_by(*order_list)
		# 5、通过分页组件实现分页功能
		...
		for obj in data_lists:  # obj-->当前的数据对象
			...
			if display_list:
				for key_or_func in display_list:
					if isinstance(key_or_func, FunctionType):
						temp_row.append(key_or_func(self, obj=obj))
					elif hasattr(obj, key_or_func):
						temp_row.append(getattr(obj, key_or_func))
			else:
				temp_row.append(obj)  # 默认添加对象，只显示其名称
		...
		return render(request, "stark/show_list.html", {
	...
	def edit_view(self, request, pk, *args, **kwargs):
		...
		:return:
		"""
		current_obj = self.model_class.objects.filter(id=pk).first()
	...
	def add_view(self, request, *args, **kwargs):
		...
		if form.is_valid():
			self.customize_save(form, is_update=False)  # 数据在保存之前可以自定制存储方法来实现
			return redirect(self.get_reverse_list_url())
		return render(request, 'stark/change.html', {"form": form})
	...
	def del_view(self, request, pk, *args, **kwargs):
		...
		:return:
		"""
		origin_url = self.get_reverse_list_url()
		if request.method == "GET":
			return render(request, "stark/del.html", {"cancel_url": origin_url})
		self.model_class.objects.filter(id=pk).delete()
		return redirect(origin_url)
	
	def get_reverse_add_url(self):
		"""用于获取由list页面到add页面的url-可保留原条件"""
		name = "%s:%s" % (self.stark_obj.namespace, self.get_url_add_name)
	...
	def get_reverse_edit_url(self, *args, **kwargs):
		...
		name = "%s:%s" % (self.stark_obj.namespace, self.get_url_edit_name)
		...
		if not self.request.GET:
			...
			new_query_dict = QueryDict(mutable=True)
			new_query_dict["_filter"] = params
			res_url = "%s?%s" % (base_url, new_query_dict.urlencode())
		return res_url
	
	def get_reverse_del_url(self, *args, **kwargs):
		"""通过此方法获取具体要跳转至显示del页面的url，保留原条件信息"""
		name = "%s:%s" % (self.stark_obj.namespace, self.get_url_del_name)
		base_url = reverse(name, args=args, kwargs=kwargs)
		if not self.request.GET:
			res_url = base_url
		else:
